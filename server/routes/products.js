...
const PRODUCT_SELECT = `
  SELECT p.id, p.name AS nome, p.price AS prezzo, c.name AS categoria, p.emoji, p.image_url
  FROM products p
...
router.post('/', (req, res) => {
  const { nome, prezzo, categoria, emoji, image_url } = req.body
  ...
  const { lastInsertRowid } = db
    .prepare('INSERT INTO products (name, price, category_id, emoji, image_url) VALUES (?, ?, ?, ?, ?)')
    .run(nome, prezzo, cat.id, emoji || '', image_url || null)
...
router.put('/:id', (req, res) => {
  ...
  db.prepare('UPDATE products SET name = ?, price = ?, category_id = ?, emoji = ?, image_url = ? WHERE id = ?')
    .run(nome, prezzo, cat.id, emoji || '', image_url || null, req.params.id)
